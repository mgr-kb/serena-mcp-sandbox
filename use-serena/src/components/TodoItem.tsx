...
export const TodoItem: React.FC<TodoItemProps> = ({
  ...
  const getPriorityDisplay = (priority: 'high' | 'medium' | 'low') => {
    switch (priority) {
      case 'high':
        return { text: '高', color: 'bg-red-100 text-red-800' };
      case 'medium':
        return { text: '中', color: 'bg-yellow-100 text-yellow-800' };
      case 'low':
        return { text: '小', color: 'bg-green-100 text-green-800' };
    }
  };
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`px-2 py-1 text-xs font-medium rounded-full ${getPriorityDisplay(todo.priority).color}`}>
